...
function detectPlatform() {
  if (get$$window().navigator.appVersion.contains("Win")) {
    $globals.osName = "win";
  }
  else if (get$$window().navigator.appVersion.contains("Mac")) {
    $globals.osName = "mac";
  }
  get$$document().query(("#" + $globals.osName)).get$attributes().$setindex("checked", "true");
}
...
function registerHandlers() {
  const$0006.forEach((function (os) {
    get$$document().query(("#" + os)).get$on().get$click().add$1((function (e) {
      $globals.osName = os;
      filterPlatformText();
    })
    );
...
var const$0006 = ImmutableList.ImmutableList$from$factory(["mac", "win", "linux"]);
var $globals = {};
$static_init();
main();
