...
#### 1.8 only: Opting in to enum support {#enum-opt-in}
{:.no_toc}

To use enums in Dart Editor, `dartanalyzer`, or `dart2js` before 1.9,
you must opt in.
The 1.8 Dart VM (`dart`) supports enums without opt-in.
...
### 1.8 only: Opting in to asynchrony support {#async-opt-in}
{:.no_toc}

To use asynchrony support in Dart Editor, `dartanalyzer`,
or the Dart VM (`dart`) before 1.9, you must opt in.

<aside class="alert alert-info" markdown="1">
**Note:**
The 1.8 Dart-to-JS compiler (`dart2js`) doesn't support asynchrony
([21411](https://code.google.com/p/dart/issues/detail?id=21411)).
However, you can try out asynchrony in a 1.8 web app by using the
[async-await pub transformer](https://github.com/dart-lang/async_await#async-await-transformer)
when building the JavaScript version of the app.
</aside>
...
return type changes to `Future<String>`:
