...
<aside class="alert alert-warning" markdown="1">
**Warning:**
Enumerated types are a new feature
first implemented in 1.8.
If you use them,
[let us know](http://dartbug.com)
about any implementation issues you discover.
</aside>

#### Opting in to enum support {#enum-opt-in}
{:.no_toc}

To use enums in Dart Editor, `dartanalyzer`, or `dart2js`, you must opt in.
The Dart VM (`dart`) supports enums without opt-in.

In Dart Editor, go to **Preferences**, click **Experimental**, and select
**Enable Enum Support**.

<aside class="alert alert-warning" markdown="1">
**Note:**
See [issue 21823](https://code.google.com/p/dart/issues/detail?id=21823)
for details on how to make Dart Editor
generate JavaScript from Dart code that uses enums.
</aside>

In `dartanalyzer` or `dart2js`, use the `--enable-enum` command-line flag:
...
`loadLibrary()` on the library's identifier.
...
The call to `then()` specifies a function to run
once the library is loaded.
This layer of indirection is necessary because the
`loadLibrary()` method is asynchronous,
returning before the library is loaded.

Another way to write the same code is to use
the new [asynchrony support](#asynchrony):
...
Dart has started adding new language features
...
they return immediately after setting up
a possibly time-consuming operation,
such as performing I/O.

When you need to use the result of one of these asynchronous functions,
...
<aside class="alert alert-warning" markdown="1">
**Warning:**
Async functions and await expressions are new features
first implemented in 1.8.
[Let us know](http://dartbug.com)
about any implementation issues you discover.
</aside>

### Opting in to asynchrony support {#async-opt-in}
{:.no_toc}

To use asynchrony support in Dart Editor, `dartanalyzer`,
or the Dart VM (`dart`), you must opt in.

<aside class="alert alert-info" markdown="1">
**Note:**
The Dart-to-JS compiler (`dart2js`) doesn't currently support asynchrony
([21411](https://code.google.com/p/dart/issues/detail?id=21411)).
...
when building the JavaScript version of your app.
</aside>

To opt into asynchrony support in Dart Editor,
go to **Preferences**, click **Experimental**, and select
...
Adding the `async` keyword to a function makes its return type Future.
...
If you change it to be an async function,
its return type changes to `Future<String>`:
...
The value of <code><em>expression</em></code> should have type Future,
indicating a promise to return an object.
The value of <code>await <em>expression</em></code> is that returned object.
The await expression makes execution pause until that object is available.

<aside class="alert alert-info" markdown="1">
**Tip:**
If `await` doesn't work,
make sure it's in an async function.
</aside>

For example, say you want to call an asynchronous function
from your app's `main()` function.
