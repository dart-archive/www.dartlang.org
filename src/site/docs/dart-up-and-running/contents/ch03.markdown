...
some or all items match a condition. {PENDING: if time permits, change
example as suggested by floitsch to have (maybe) cities instead of
isDecaffeinated.}
...
components of a URI—host, port, scheme, and so on.{PENDING: show
...
For more information, see ? and the [Exception API
...
separate lines. These transformers are from the dart:convert library
(see ?).{PENDING: add onDone and onError. (See "Streaming file
contents".)}
...
-   [Use Future-Based
    APIs](https://www.dartlang.org/docs/tutorials/futures/)

-   [Futures and Error
    Handling](http://www.dartlang.org/articles/futures-and-error-handling/)

-   [The Event Loop and
    Dart](http://www.dartlang.org/articles/event-loop/)

-   [Use Streams for
    Data](https://www.dartlang.org/docs/tutorials/streams/)

-   [Creating Streams in
    Dart](http://www.dartlang.org/articles/creating-streams/)
...
methods. Also see the API docs
for[num,](http://api.dartlang.org/dart_core/num.html)
...
> [Polymer.dart](http://www.dartlang.org/polymer-dart/) and
...
Add an event handler using `element.on.listen()`, where `` is the event
name and `` is the event handler.
...
Code Samples.](http://www.dartlang.org/samples/)
...
Samples.](http://www.dartlang.org/samples/) Dart has additional
...
For a more comprehensive HTTP server, see ?. It features the Dartiverse
...
Mirrors](https://www.dartlang.org/articles/reflection-with-mirrors/) has
